...
if __name__ == '__main__':
    ...
    if args.fast_nms == True:
        print(f'{num_count}, args.fast_nms')
        num_count = num_count + 1
    if args.cluster_nms == True:
        print(f'{num_count}, args.cluster_nms')
        num_count = num_count + 1
    if args.cluster_diounms == True:
        print(f'{num_count}, args.cluster_diounms')
        num_count = num_count + 1
    if args.spm == True:
        print(f'{num_count}, args.spm')
        num_count = num_count + 1
    if args.spm_dist == True:
        print(f'{num_count}, args.spm_dist')
        num_count = num_count + 1
    if args.spm_dist_weighted == True:
        print(f'{num_count}, args.spm_dist_weighted')
        num_count = num_count + 1
    if num_count > 1:
        raise Exception(
            "You must choose one NMS strategy. Options: fast_nms, cluster_nms, cluster_diounms, spm, spm_dist, spm_dist_weighted.")
